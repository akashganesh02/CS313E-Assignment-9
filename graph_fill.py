...
class ImageGraph:
    ...
    def bfs(self, start_index, color):
        ...
        #Check initial node and color
        start_node = self.nodes[start_index]
        start_color = start_node.color

        #Set of indices that have been visited or enququed
        enququ_or_visit = set()
        enququ_or_visit.add(start_index)

        #initialize a queue and enqueue the starting node
        q1 = Queue()
        q1.enqueue(start_index)

        while not q1.is_empty():
           current_index = q1.dequeue() #dequque a node from the front of the queue
           current_node = self.nodes[current_index]

           #If node not visited yet, explore it
           if not current_node.visited and current_node.color == start_color:
              current_node.visit_and_set_color(color) #set color and mark node as visited
              self.print_image()

           #Enquque neighboring nodes that have not been visited
           for neighbor_index in current_node.edges:
              neighbor = self.nodes[neighbor_index]
              if neighbor_index not in enququ_or_visit and neighbor.color == start_color:
                 q1.enqueue(neighbor_index)
                 enququ_or_visit.add(neighbor_index)
    ...
    def dfs(self, start_index, color):
        ...
        #Check initial node and color
        add_or_visit = set()
        add_or_visit.add(start_index)

        #initialize stack and add starting node to stack
        s1 = Stack()
        s1.push(start_index)

        while not s1.is_empty():
           current_index = s1.pop()
           current_node = self.nodes[current_index]

           #If node not yet visited, explore it
           if not current_node.visited and current_node.color == start_color:
              current_node.visit_and_set_color(color)
              self.print_image()

            #Push neighboring nodes that have not been visited
           for neighbor_index in current_node.edges:
              neighbor = self.nodes[neighbor_index]
              if neighbor_index not in add_or_visit and neighbor.color == start_color: #self.nodes[neighbor_index].visited:
                s1.push(neighbor_index)
                add_or_visit.add(neighbor_index)
...
def create_graph(data):
    ...
    for i in range(edge_count):
        ...
        graph.adjacency_matrix = [[0 for _ in range(node_count)] for _ in range(node_count)]

        #populate the adjacency matrix based on edges
        for node in graph.nodes:
           for edge in node.edges:
              graph.adjacency_matrix[node.index][edge] = 1
